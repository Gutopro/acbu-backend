 ...
 */
export async function sendXlmToActivate(stellarAddress: string): Promise<string> {
  ...
  try {
    ...
    if (msg.includes('op_already_exists') || msg.includes('CREATE_ACCOUNT_ALREADY_EXIST')) {
      logger.info('Wallet already funded, skip activation', { stellarAddress: stellarAddress.slice(0, 8) + '…' });
